...
CalorimeterConstructionDummyOptical::CalorimeterConstructionDummyOptical()
...
void CalorimeterConstructionDummyOptical::ComputeObjectsPositioning(){
...
void CalorimeterConstructionDummyOptical::SetNumberOfCrystalLayer(G4int aVal){
...
void CalorimeterConstructionDummyOptical::Builder(G4VPhysicalVolume* motherVolume)
{
  ...
  if(fNbOfCrystalLayers>0){
    ...
    //new G4LogicalSkinSurface("MylarSurface",fLogicLayer,matMylarSurface);
  }
  ...
  //new G4LogicalSkinSurface("MylarSurface",fLogicScintLayer,matMylarSurface);
  ...
  if(fNbOfCrystalLayers>0){
    ...
    fPhysiScintLayer = new G4AssemblyVolume();
    G4RotationMatrix Ra;
    G4ThreeVector   Ta, Tb;
    G4double startZ = -fScint_Z*0.5 + fCaloLayer_Z*0.5 ;
  ...
  G4double mylar_Energy[2] = {2.0*eV, 3.6*eV};
  G4double mylar_REFL[2] = {0.9,0.9};
  G4double mylar_EFFI[2] = {0.0, 0.0};
  MPTmylarSurface->AddProperty("REFLECTIVITY",mylar_Energy,mylar_REFL,2);
  MPTmylarSurface->AddProperty("EFFICIENCY",mylar_Energy,mylar_EFFI,2);
     matMylarSurface->SetMaterialPropertiesTable(MPTmylarSurface);

   new G4LogicalSkinSurface("scintWrapping",fLogicScintLayer,matMylarSurface);
    for (int il = 0 ; il < fNbOfScintLayers; il++)
    {
        
        Ta.setZ(startZ + il * fCaloLayer_Z);
        fPhysiScintLayer->AddPlacedVolume(fLogicScintLayer, Ta, &Ra);
    }


    
    fPhysiScintLayer->MakeImprint(fLogicScintBox, Tb ,&Ra);


     fPhysiScintBox = new G4PVPlacement(0,                        
				       G4ThreeVector(fPhysiScintBox_X,fPhysiScintBox_Y,fPhysiScintBox_Z /*- fScint_Z/2. + fCaloLayer_Z*0.5*/),   
				       "CalorimeterScint",               
				       fLogicScintBox,                
				       fPhysiCaloBox,
				       false,                       
